...
$affiliationArray = mysql_fetch_array($resultAffiliation);
  ...
  if(empty($affiliationID)) { ?>  
    ...
    <div class="clearfix" style="margin-bottom: 5px;">
      <a href="addAffiliation.php" style="margin-left: 5px"; class="pull-right btn">Add Affiliation <i class="icon-plus-sign"></i></a>
    </div>
   ...
   <?php
         ...
         <!-- delete button -->
         <button rel="tooltip" onClick="setConfirmUnload(false);" title="Delete the affiliation" type="submit" id="delete" class="btn btn-danger" name="delete" value="delete" ><i class="icon-white icon-remove"></i></button>
       ...
       while($affiliationArch = mysql_fetch_array($resultArch)) 
       {
          ...
          <form class="form-horizontal" name="activate" id="activate" method="post" enctype="multipart/form-data" action="functions/doEditAffiliation.php"> 
           ...
           <input type="hidden" class="span3" id="affilation" name="affiliation" class="textfield"  value="<?= $affiliationArch["strAffiliation"];?>" /> 
            ...
            <!-- activate button -->
           <button rel="tooltip" onClick="setConfirmUnload(false);" title="Activate the affiliation" type="submit" id="activate" class="btn btn-success" name="activate" value="activate" ><i class="icon-white icon-ok"></i></button>
      ...
      <?php 
        ...
        <h3>Edit an Affiliation</h3>
          ...
          <div class="control-group <?php if($_SESSION['affiliationError'] == 1) echo 'error'; ?>">
             ...
             <div class="controls">
               <input type="text" class="span3" id="affiliation" placeholder="Affiliation" name="affiliation" value="<?= htmlspecialchars(stripslashes($affiliationArray["strAffiliation"]), ENT_QUOTES);?>" > <span class="inline-help text-error">Required</span>
