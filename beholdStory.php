...
<?php
  if(empty($row["txtIntent"])) {
  	echo "<div class='alert alert-warning'><i class='icon-flag'></i> <strong>Attention:</strong> This story does not appear in the work list. <br/>You must specify an intent for your story to be listed in the work list.</div>";

  }
?>


	<h2><?php if($row["isTopStory"] == 1 || $row["isExtTopStory"] == 1) echo "<span class='label label-inverse' style='position: relative; top: -5px;'><i class='icon-star icon-white' rel='tooltip' title='This was featured as a Top Story. '></i></span>"?> <?php if(!empty($row["strHeadline"])) { echo htmlspecialchars(stripslashes($row["strHeadline"]), ENT_QUOTES); } else { echo htmlspecialchars(stripslashes($row["strFilename"]), ENT_QUOTES); } ?></h2>
	

	<div class="well" style="float: left; width: 94%;"> 
						...
						<?php  
							while($writers = mysql_fetch_array($result5)) {
									if(!empty($writers["strFirstName"])) {
										echo "<i class='icon-user' rel='tooltip' title='Phone: " . $writers["strPhone"] . "'></i> <a href='mailto:" . $writers["strEmail"] . "'>" . $writers["strFirstName"] . " " . $writers["strLastName"] . "</a><br />";
									}
								}
						?>
					...
					<td colspan="2">
						<?php
							if($row["datePublished"] != "0000-00-00") {
								...
								echo "Expected Release: <a href='editStory.php?newsID=" . $_GET["newsID"] . "#pubDate' class='btn btn-mini'><i class='icon-plus-sign'></i> Add Publish Date</a>";
							}
						?>
					...
					<td style="border: none;">
						<?php 
							if(!empty($row["strReach"])) {
								...
								echo "<a href='editStory.php?newsID=" . $_GET["newsID"] . "#reach' class='btn btn-mini'><i class='icon-plus-sign'></i> Add Reach</a>";
							}
						?>
					</td>
				</tr>
					...
					<td>
						<?php 
							if(!empty($row["txtIntent"])) {
								...
								echo "<a href='editStory.php?newsID=" . $_GET["newsID"] . "#intent' class='btn btn-mini'><i class='icon-plus-sign'></i> Add Intent</a>";
							}
						?>
					</td>
				</tr>
	...
	<div class="well" id="facts" style="float: left; width: 95%; clear:both;">
					...
					if($sourceCount > 0) {
						while($sources = mysql_fetch_array($result4)) {
								if(!empty($sources["strFirstName"])) {
									echo "<i class='icon-user' rel='tooltip' title='Phone: " . $sources["strPhone"] . "'></i> <a href='mailto:" . $sources["strEmail"] . "'>" . $sources["strFirstName"] . " " . $sources["strLastName"] . "</a><br />";
								}
							}
					}
					else {
						echo "<a href='editStory.php?newsID=" . $_GET["newsID"] . "#sources' class='btn btn-mini'><i class='icon-plus-sign'></i> Add Sources</a>";
					}
			...
			<tr>
				...
				<td><?php  
					if($topicCount > 0) {
						...
						echo "<a href='editStory.php?newsID=" . $_GET["newsID"] . "#topics' class='btn btn-mini'><i class='icon-plus-sign'></i> Add Topics</a>";
					}
			...
			<tr>
				...
				<td><?php 	
					...
					if($issueCount > 0) {
						...
						echo "<a href='editStory.php?newsID=" . $_GET["newsID"] . "#issues' class='btn btn-mini'><i class='icon-plus-sign'></i> Add Issues</a>";
					}
			...
			<tr>
				...
				<td>
					<?php
						...
						if(!$website) {
								echo "<a href='editStory.php?newsID=" . $_GET["newsID"] . "#websites' class='btn btn-mini'><i class='icon-plus-sign'></i> Add Websites</a>";
						}
					?>
				</td>
			</tr>
			...
			<tr>
				...
				<td><?php 
					...
					if($deptCount > 0) {
						...
						echo "<a href='editStory.php?newsID=" . $_GET["newsID"] . "#depts' class='btn btn-mini'><i class='icon-plus-sign'></i> Add Departments</a>";
					}
			...
			<tr>
				...
				<td><?php 
					...
					if($affiliationCount > 0) {
						...
						echo "<a href='editStory.php?newsID=" . $_GET["newsID"] . "#affiliations' class='btn btn-mini'><i class='icon-plus-sign'></i> Add Affiliations</a>";
					}
					?></td>
			</tr>
			...
			<tr>
				...
				<td><?php 
					if($areaCount > 0) {
						...
						echo "<a href='editStory.php?newsID=" . $_GET["newsID"] . "#areas' class='btn btn-mini'><i class='icon-plus-sign'></i> Add Areas</a>";
					}
					?></td>
			</tr>
...
<div class="well" style="width: 30%; min-height: 300px; padding: 10px; margin-left:15px; float: left;">
	...
	<br />
	<a href='editStory.php?newsID=<?php echo $_GET["newsID"] ?>' class='btn btn-block btn-warning'><i class='icon-white icon-edit'></i> Edit Story</a>
	<a href='functions/doSaveStory.php?newsID=<?php echo $_GET["newsID"] ?>' class='btn btn-block btn-success'><i class='icon-download-alt icon-white'></i> Download Story</a>
	<br />
	<div class="well" style="border: none; margin-left: -5px; margin-right: -5px;">
	   ...
	   <?php
		...
		<form class="form-horizontal" id="stageForm" style="margin: 0px;" action="functions/doUpdateStage.php" method="post">
			<div class="control-group">
					...
					<a onClick="doSourceCheck()" rel="tooltip" title="Update this story's progress." class="btn btn-small btn-success"><i class='icon-ok icon-white' ></i></a>
		...
		<div id="sourceModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		  ...
		  <div class="modal-body">
		  	<p class="span5"><span class="label label-info"><i class="icon-ok icon-white"></i></span> If you wish to send a note to your source along with your story simply type your message below.</p>
		  	<p class="span5"><span class="label label-info"><i class="icon-remove icon-white"></i></span> If you wish to just send your story, leave the field below blank.</p>
		...
		<div class="progress progress-success progress-striped active">
		  <div class="bar" style="width: <?php echo ($row["stageID"]/$stageC["stageCount"])*100 ?>%"> <?php echo number_format(($row["stageID"]/$stageC["stageCount"])*100, 0, '.', ' ') ?>%</div>
		</div>
	</div>
...
<?php 
	...
	echo "<div class='alert alert-error alert-block'><h4>Houston, we have a problem!</h4><p>The ID that you provided was not found in the database. It either never existed or was deleted.</p></div>";
}
?>
