...
<p>Please note: if you edit a topic, that topic will be edited throughout all recorded stories.</p><?php 
             if($_SESSION['topicAddError'] == 1) {
               echo "<div class='alert alert-error'><i class='icon-exclamation-sign'></i> Looks like you forgot to enter an topic. Please enter an topic before continuing.</div>";
             } 
?>

     <!-- New form: addForm01 -->     
   <form class="form-horizontal" name="addForm01" id="addForm01" method="post" enctype="multipart/form-data" action="functions/doTopic.php"> 
      
         <!-- New Issue: client side error if field is blank upon "Submit" need to be required-->
      <div class="control-group <?php if($_SESSION['topicAddError'] == 1) echo 'error'; ?>">
    
         	  <label class="control-label" for="topicAdd">Add a new Topic</label>
          ...
          <div class="controls"> 
            <div class="input-append"> 
                 ...
                 <input type="text" class="span3" id="topicAdd" name="topicAdd" class="textfield" placeholder="Issue" required ="required" value="<?= htmlspecialchars(stripslashes($_SESSION['topicAdd']), ENT_QUOTES);?>" /> 
...
?>
 ...
 <form class="form-horizontal" name="addForm02" id="addForm02" method="post" enctype="multipart/form-data" action="functions/doTopic.php"> 

    <!-- Edit or Delete: client side error if field is blank upon "Submit"-->
   <div class="control-group <?php if($activity['strTopic'] == 1) echo 'error'; ?>" >
               
      <div class="controls">
         <div class="input-append">           
                      <!-- **07-27: USE the htmlspecialchars(stripslashes ...) to show the content without the escape backslash (\). This would show if there were an error on the page -->
                      <input type="text" class="span3" id="topicEdDel" name="topicEdDel" class="textfield" required="required" value="<?= htmlspecialchars(stripslashes($activity['strTopic']), ENT_QUOTES);?>" /> 
         
         <input type="hidden" class="span3" id="topicID" name="topicID" class="textfield"  value="<?= $activity['topicID'];?>" /> 
         ...
         <input type="hidden" class="span3" id="isHidden" name="isHidden" class="textfield"  value="<?= $activity['isHidden'];?>" /> 
            ...
            <span id="topicControls">                
              ...
              <!-- edit button -->     
             <button rel="tooltip" onClick="setConfirmUnload(false);" title="Edit the topic" type="submit "id="edit" class="btn" name="edit" value="edit" onclick="Clicked(this)">Edit <i class="icon-edit"></i></button>

              <!-- delete button -->
              <button rel="tooltip" onClick="setConfirmUnload(false);" title="Delete the topic" type="submit" id="delete" class="btn btn-danger" name="delete" value="delete" ><i class="icon-white icon-remove"></i></button>
...
$i++;
 ...
 <form class="form-horizontal" name="addForm03" id="addForm03" method="post" enctype="multipart/form-data" action="functions/doTopic.php" onsubmit="return show_reSubmit()" > 
   
    <!-- Activate: client side error if field is blank upon "Submit"-->
    <div class="control-group <?php if($activityDelete['strTopic'] == 1) echo 'error'; ?>" >
                      
        <div class="controls">
         <div class="input-append">           
                      <!-- **07-27: USE the htmlspecialchars(stripslashes ...) to show the content without the escape backslash (\). This would show if there were an error on the page -->
                      <input type="text" class="span3" id="topicActivate" name="topicActivate" class="textfield" required="required" value="<?= htmlspecialchars(stripslashes($activityDelete['strTopic']), ENT_QUOTES);?>" /> 
         
         <input type="hidden" class="span3" id="topicID" name="topicID" class="textfield"  value="<?= $activityDelete['topicID'];?>" /> 
